...
void BattleLevel::On_Update(float deltaTime)
{
...
void BattleLevel::BeginPlay()
{
	...
	background0->GetComponentByClass<SpriteRenderer>()->SetFilter(true, BLACK, Math::RandInt(50, 100));
	background0->GetComponentByClass<SpriteRenderer>()->EnableFilter();
	/*background0->GetComponentByClass<SpriteRenderer>()->MeanBlur(10);*/
	/*background0->GetComponentByClass<SpriteRenderer>()->WaveFrom(100,500,1500,2550);*/
	/*test.Bind(1, [this]() {
		background0->GetComponentByClass<SpriteRenderer>()->WaveFrom(50, 200, 150, 255);
	});*/
	...
	/*GameStatics::CreateObject<Enemy>(Vector2D(300, 200));*/

	/*sunflower=GameStatics::CreateObject<SunFlower>(Vector2D(310, 500));*/
	/*SignalSlotSystem::CONNECT(mainController, &BattleController::PlantSeed, sunflower, Produce());*/
	...
	for (int i = 0; i < 20; i++)
	{
		ember = GameStatics::CreateObject<Particle_ember>({ static_cast<float>( Math::RandReal(0,200)),static_cast<float>(Math::RandReal(500,600)) });
		/*GameStatics::CreateObject<Particle_smoke>({ static_cast<float>(Math::RandReal(200,800)),static_cast<float>(Math::RandReal(500,600)) });*/
		/*ember->SetLocalScale(Vector2D(5,5));*/
	}
}
