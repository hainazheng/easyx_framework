...
bool CircleCollider::IsMouseOver()
{
    ...
    return Vector2D::Distance(GetWorldPosition(), GameStatics::GetController()->GetMousePosition())<=radius;
}

void CircleCollider::On_Update(float deltaTime)
{
    Collider::On_Update(deltaTime);
    //近似一个圆
...
{
    Vector2D pos = GetWorldPosition(); Vector2D pos_m = GameStatics::GetController()->GetMousePosition();
    return pos_m.x >= pos.x - size.x / 2 && pos_m.x <= pos.x + size.x / 2
        &&pos_m.y >= pos.y - size.y / 2 && pos_m.y <= pos.y + size.y / 2;
}

void BoxCollider::On_Update(float deltaTime)
{
    Collider::On_Update(deltaTime);
...
{
    return Vector2D((std::min)(rect1.right,rect2.right)-(std::max)(rect1.left,rect2.left), (std::min)(rect1.bottom,rect2.bottom)-(std::max)(rect1.top,rect2.top));
}
 ...
 * 
 */
void Collider::On_Update(float deltaTime)
{
    SceneComponent::On_Update(deltaTime);
...
HitResult Collider::CollisionHitBoxToBox(Collider*  me, Collider* another)
{
    ...
    float x = (std::max)(box1->GetRect().left, box2->GetRect().left) + overlaprect.x / 2;
    float y = (std::max)(box1->GetRect().bottom, box2->GetRect().bottom) + overlaprect.y / 2;
