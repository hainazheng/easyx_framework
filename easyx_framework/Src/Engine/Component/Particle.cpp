...
void Particle::produce()
{
	...
	if (pattern == ParticlePattern::Center)
	{
		...
		if (radius != Vector2D(0, 0)) { temp.offset = unitVector * (std::min)( radius.x,radius.y) + abs(radius.y - radius.x) * unitVector * Math::RandPerc(); }
...
void Particle::On_Update(float deltaTime)
{
...
void Particle::Render()
{
	...
	if (!bIsEnabled || !images)return;
		...
		for (auto& each : particle)
		{
			...
			BLENDFUNCTION bf = { AC_SRC_OVER,0,static_cast<BYTE>(each.alpha),AC_SRC_ALPHA };				//设置透明度，成员全是BYTE类型
			...
			if (!each.isOnFilter && bFilterEnable && FilterLayer.size() > 0)
			{
				//第一次生产才添加滤镜
				each.isOnFilter = true;
				FilterImage(images[each.index]);
			}
			if (bFilterEnable && filter && FilterLayer.size() > 0)
			{
				HDC srcDC_filter = GetImageHDC(filter);
				AlphaBlend(dstDC, (int)pos.x , (int)pos.y , dw, dh, srcDC_filter, 0, 0, w, h, bf);			//相当于putimage 粘贴到相应的工作平面
			}
			else
