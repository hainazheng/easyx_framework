...
void SpriteRenderer::Render()
{
	...
	//跟弹簧长度成反比实现全局缩放
	//物体实际大小
	int dw = int(w * std::abs(GetWorldScale().x) * 20.f / mainWorld.mainCamera->springArmLength_virtual);
	int dh = int(h * std::abs(GetWorldScale().y) * 20.f / mainWorld.mainCamera->springArmLength_virtual);
	...
	AlphaBlend(dstDC, (int)pos.x - dw / 2, (int)pos.y - dh / 2, dw, dh, srcDC, (int)spriteinfo.StartLoc.x, (int)spriteinfo.StartLoc.y, w, h, bf);			//相当于putimage 粘贴到相应的工作平面
...
///执行支持的旋转滤镜等功能
void SpriteRenderer::On_Update(float deltaTime)
{
...
void SpriteRenderer::SaveImage(IMAGE* img)const
{
	if (!img)return;
	std::string path = "SaveGame/ProcessedImage/" + GetName() + ".png";
