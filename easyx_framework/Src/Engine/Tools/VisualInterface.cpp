...
void ImageInterface::RotateImage(double radian)
{
	if (!sprite)return;
		...
		for (int i = 0; i < 4; i++)
		{
			...
			//旋转后图像的大小
			min_x = (std::min)((long)min_x, point[i].x);
			max_x = (std::max)((long)max_x, point[i].x);
			min_y = (std::min)((long)min_y, point[i].y);
			max_y = (std::max)((long)max_y, point[i].y);
		}
...
///给图片施加滤镜
///滤镜实现
void ImageInterface::FilterImage(IMAGE* img)
{
	if(!img)
	img = copy ? copy : sprite;
	if (!img)return;
...
void ImageInterface::WaveFrom(int x, int y, int stonesize, int stoneweight)
{
	...
	ImageToolkit::putstone(sprite, wave, x, y, stonesize, stoneweight, wave1, wave2);

	//控制更新振幅的速度
	WaveUpdate.Bind(1, [this]() {
		if (!wave)return;
		if (bIsWave)
		{
			ImageToolkit::calcnextframewave(sprite, wave,wave1,wave2);
			ImageToolkit::calcimage(sprite, wave, wave1, wave2);
			if (WaveUpdate.GetTime() >= 6.0)
			{
				ExitWave();
			}
...
void ImageToolkit::putstone(IMAGE* srcImg, IMAGE* dstImg, int x, int y, int stonesize, int stoneweight,std::vector<short>& wave1, std::vector<short>& wave2)
{
	...
	calcnextframewave(srcImg, dstImg,wave1,wave2);
	calcimage(srcImg, dstImg, wave1, wave2);
...
void ImageToolkit::MeanFilter(IMAGE* srcImg, IMAGE* dstImg, int radius)
{
	...
	for (int i = 0; i < height; ++i) {
		for (int j = 0; j < width; ++j) {
			int x = i + 1, y = j + 1;
			int x1 = (std::max)(x - w - 1, 1), y1 = (std::max)(y - w - 1, 1);
			int x2 = (std::min)(x + w, height), y2 = (std::min)(y + w, width);
...
void ImageToolkit::GaussianFilter(IMAGE* srcImg, IMAGE* dstImg, int radius)
{
	...
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++) {
			...
			temp[y * width + x] = RGB(Math::Clamp((int)r,0,255), Math::Clamp((int)g, 0, 255), Math::Clamp((int)b, 0, 255));
		}
	}
	...
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++) {
			...
			pNewBuf[y * width + x] = (pBuf[y * width + x] & 0xFF000000) | RGB(Math::Clamp((int)r, 0, 255), Math::Clamp((int)g, 0, 255), Math::Clamp((int)b, 0, 255));
		}
	}
...
void ImageToolkit::calcnextframewave(IMAGE* srcImg, IMAGE* dstImg,std::vector<short>& wave1, std::vector<short>& wave2)
{
	int width = srcImg->getwidth(), height = srcImg->getheight();
	auto GetIndex = [=](int i, int j)->int
		{
...
void ImageToolkit::calcimage(IMAGE* srcImg, IMAGE* dstImg, std::vector<short>& wave1, std::vector<short>& wave2)
{
