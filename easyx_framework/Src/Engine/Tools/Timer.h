...
public:
	...
	Timer():stopTime (nanoseconds(0)), startTime(nanoseconds(0)) {
	...
	)	
	{
		...
		if (endDelay >= 0)  resetTime = milliseconds(int(1000 * endDelay));
	...
	void Execute()			
	{
		...
		//回收计时器，防止计时器一直在执行，浪费内存
		if (resetTime.count()>0&&GetTime() >= resetTime.count())
		{
            mainWorld.GameTimers_to_delete.insert(this);
			ReSet();
		}
	}
	...
	///获取计时器实际总执行时间
	double GetTime() { return getTime().count(); };
	...
	///重置时间点，重新开始计时
	void ReSet() { 
		lastTime = steady_clock::now(); startTime = lastTime; bpersistent = false;
		bRunning=true;stopTime =time_point<steady_clock>(nanoseconds(0));
		resetTime = duration<double>(0);delay = duration<double>(0);
	...
	void Continue()
	{
		//防止重复更新lastTime
		if (bRunning)return;
		bRunning = true; lastTime += (steady_clock::now() - stopTime); startTime += (steady_clock::now() - stopTime);
	}

private:
	using TimerEvent= std::function<void()>;
	TimerEvent callback;		///<把函数指针进行适配，也就是包装
	...
	time_point<steady_clock> startTime;						///<开始时间点
    duration<double> resetTime = duration<double>(0);		///<重置时间		
