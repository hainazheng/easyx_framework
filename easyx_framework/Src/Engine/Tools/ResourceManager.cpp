...
{
	num=num> row * col ? row * col : num;	//防止用户输入错误
...
void ResourceManager::Load(
	...
	, std::string path		///<[in]动画路径
	, Pair Size				///<[in]图像尺寸
	, Pair start			///<[in]图像开始位置
	, Pair end				///<[in]图像结束位置
	, int num				///<[in]总共数量
	, int row				///<[in]最大多少行
	, int col				///<[in]最大多少列
	, bool r_priority			///<[in]优先级,默认列优先，即列数不变，行变化
)
{
	IMAGE** images = new IMAGE * [num];
	IMAGE img;
	loadimage(&img, (LPCTSTR)path.c_str(), Size.x, Size.y);
	
	DWORD* src = GetImageBuffer(&img);
	


	int width = (end.x-start.x)/ col;		//获取单场图片的宽度
	int height = (end.y - start.y) / row;		//获取单张图片的高度
	//#define Elem(array,_row,_col)  (array[(_row)*WIN_WIDTH+(_col)])
	for (int i = 0; i < ((!r_priority)? row:col); i++)
	{
		for (int j = 0; j < ((!r_priority) ? col:row); j++)
		{
			//当前要处理图片的坐标
			int x = j * width;
			int y = i * height;
			...
			for (int x1 = 0; x1 < width; x1++)
			{
				for (int y1 = 0; y1 < height; y1++)
				{
					int pNewBufA = (src[x + x1 + start.x + (y + y1 + start.y) * img.getwidth()] & 0xFF000000) >> 24;

					dst[x1 + (y1) * width] = (pNewBufA << 24) | (src[x + x1 +start.x+ (y + y1+start.y) * img.getwidth()] & 0xFFFFFF);
				}
			}
		}
	}
...
void ResourceManager::Initialize()
{
	...
	Load("ember_particle", "res/Particles/ember_particle.png", 45, 45, 1, 1, 1);
    Load("smoke_particle_", "res/Particles/wispy_smoke_particle_abyss.png", 102, 512, 5, 5, 1);

	Load("enemy1", "res/enemy/enemy1_idle", 100, 100, 7);
	/*Load("enemy1", "res/enemy/enemy1.png", Pair(1049, 601),Pair(206, 0), Pair(990, 98), 7, 1, 7);*/
}
