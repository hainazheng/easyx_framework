...
protected:			//继承对象可以访问
	...
	void FilterImage(IMAGE* img=nullptr);

	IMAGE* blur=nullptr;			///<滤波之后的句柄

	//波幅信息
	IMAGE* wave = nullptr;			///<波幅句柄
	bool bIsWave=false;		///<是否开启波幅
	Timer WaveUpdate;					///<水波纹计时器

	std::vector<short>wave1;		///<当前时刻的振幅数据
	std::vector<short>wave2;		///<上一时刻的振幅数据
public:
	///析构函数 
	/// 释放所有开辟的图像句柄资源
...
public:
	 ...
	 * \param [in] stonesize	    石头大小
	 * \param [in] stoneweight		石头重量
	 * \param [in] wave1		  上一帧的振幅数据
	 * \param [in] wave2		  下一帧的振幅数据
	 */
	static void putstone(IMAGE* srcImg, IMAGE* dstImg,int x, int y, int stonesize, int stoneweight,std::vector<short> &wave1, std::vector<short>& wave2);
	 ...
	 * \param [in] wave2		  下一帧的振幅数据
	 */
	static void calcnextframewave(IMAGE* srcImg, IMAGE* dstImg, std::vector<short>& wave1, std::vector<short>& wave2);
	 ...
	 * \param [in] wave2		  下一帧的振幅数据
	 */
	static void calcimage(IMAGE* srcImg, IMAGE* dstImg,std::vector<short>& wave1, std::vector<short>& wave2);
