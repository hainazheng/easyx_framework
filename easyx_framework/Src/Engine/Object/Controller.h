...
{
	std::shared_ptr<InputComponent> inputcomponent;			///<输入交互组件 不希望派生类随便访问
protected:
	 ...
	 */
	virtual void SetupInputComponent(std::shared_ptr<InputComponent> inputcomponent) ;			
public:
	Controller();

	virtual ~Controller() { 
		
		if (mainWorld.mainCamera == camera)mainWorld.mainCamera = nullptr;
		UnregisterComponent(inputcomponent.get());
	};
	...
	///更新点输入信息
	void PeekInfo() { if (!inputcomponent)return; inputcomponent->PeekInfo(); }
