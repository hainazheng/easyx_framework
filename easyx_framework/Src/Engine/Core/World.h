...
class World final			
{
	...
	friend class ZHN::Log;
	friend class Configurator;
	friend class FrameRateConfig;


	std::mutex updateMutex;		///<互斥锁 防止某一个时刻耗时过多，导致下一次执行冲突 会一直等待防止顺序的未知
	std::mutex globeMutex;		///<全局锁 防止在创建类的时候，其他线程访问
    ...
    ///帧率配置器
	class FrameRateConfig* frameRateConfig = nullptr;
	...
	std::unordered_set<class Timer*>GameTimers_to_delete;			///<删除计时器容器缓冲区
