 ...
 */
class base:public SignalSlotSystem::SignalObject
{
...
public:
	...
	virtual void On_Update(float deltaTime) {};		//纯虚函数  所有的类都希望统一用update执行操作
...
public:
	...
	Level() {  mainController = nullptr; }
	Level(int width, int height) {  mainController = nullptr; }
	~Level() { mainController = nullptr; }
	/**
	 * 务必把关卡初始化逻辑写进BeginPlay，禁止写入构造函数.
	 */
	virtual void BeginPlay() {/*Resize(NULL, WIN_WIDTH, WIN_HEIGHT);*/};
