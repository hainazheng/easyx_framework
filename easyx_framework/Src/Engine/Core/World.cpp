...
///实现世界类容器所有想要执行的操作
void World::On_Update(float deltaTime)
{
	ProcessColliderZones();

	CurrentLevel->On_Update(deltaTime);
	for (auto& obj : GameObjects)
	{
		obj->On_Update(deltaTime);
	}
	{
	...
	}
	{
		...
		for (auto& obj : GameTimers_to_delete)
		{
			GameTimers.erase(obj);
			/*delete obj;*/
		}
	}
...
void World::Update_()
{
	if(mainCamera)mainCamera->Calculate();
	

	/*CurrentLevel->Update();*/
	GameStatics::GetController()->MouseTick();
}
...
void World::Input()
{
	std::lock_guard<std::mutex> lock(globeMutex);
	//没有控制者就会创建 按着单独判断
...
{
	if(levelManager)levelManager->RefreshLevel();
}
...
void World::ClearWorld()
{
	...
	GameTimers.clear();
	GameRenderers.clear();
	/*delete GameStatics::GetController();*/
