...
namespace ZHN
{
	...
	public:
		...
		static void Init()
		{
			...
			mainWorld.frameRateConfig = FrameRateConfig::instance();
			//初始化碰撞管理器
			...
			mainWorld.levelManager->RefreshLevel();
		...
		static void Tick(float deltaTime)
		{
			...
			mainWorld.On_Update(deltaTime);		//这里的deltatime是上一帧的时间间隔
			mainWorld.Render();
			mainWorld.TransForm();
		}
		...
		static void Tick_()
		{
			if (!GameStatics::GetController())return;
			{
		...
		static void Run()
		{
			...
			while (true)
			{
				...
				Application::Tick(mainWorld.deltaTime);
				if (mainWorld.frameRateConfig->IsFrameLimitOn())
				{
					double interval = double(1) / double(mainWorld.frameRateConfig->GetMaxFrame()) - (GameStatics::GetTimeSeconds() - lastTime) - 0.001;		//每一帧最少时间-这一帧执行持续时间-1秒千帧的单帧时间（最少的一帧时间）=这一帧剩下的时间
...
{
	ZHN::Application::Tick_();
}

int main()
{
	ZHN::Application::Init();
